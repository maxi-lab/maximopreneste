import py from '../assets/img/python.webp'
import poo from '../assets/img/POO.png'
export const expirience = [
    {
        title: "Ayudantia de Sintaxis y Semantica de los Lenguajes",
        t:"Syntax and Semantic of Languages",
        description: "Me encargo de correccion de Trabajos Practicos de Tipos de Datos Abstractos (en Python) y de resolucion de consultas de TAD y automatas.",
        img: py,
        altImg: "En la catedra se utiliza Python",
        alternative:"Python",
        desc:"I correct projects of abstract data type. The students have to use Python. Moreover, I aswer questions about ADT and automata theory.",

    },
    {
        title: "Ayudantia de Paradigmas de Programacion ",
        t:"Programing Paradigms helper",
        description: "Me encargo de correccion de Trabajos Practicos de Programacion Orientada a Objetos (en Samalltalk) y de resolucion de consultas de Programacion Funcional (Haskell) y Logica (Prolog).",
        img: poo,
        altImg: "En la catedra se utiliza tambien UML",
        alternative:"UML",
        desc:"I correct projects of object-orientated programing. The students have to use Smalltalk. Moreover, I aswer questions about OOP, functional progaming (Haskel) and logical paradigm (Prolog).",
    }
    
]